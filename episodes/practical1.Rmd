...
We used the scans from the patient `CT-PET-VI-02`. In the zipped data folder, you can find it under `practical1`. You should have: 
...
When using open datasets that contain images of humans, such as those from TCIA, for your research, you will still need to get ethical approval as you do for any non-open datasets you use). This can usually be obtained from the local ethics committee.

::::::::::::::::::::::::::::::::::::::

The first part of this tutorial makes use of the ITK-SNAP software. Make sure you have installed it following the instructions from the Summary and Setup page. For second part is based in Python, and you should use the template Jupyter notebook *practical1-exercises.ipynb*. 
...
However, the NIfTI format does not require that the `sform` and `qform` specify the same matrix, and it is not well defined what should be done when they are both provided and are different from each other. Therefore, we choose to use the `sform` and ignore the `qform`, which we set to unknown using the `set_qform()` function.
