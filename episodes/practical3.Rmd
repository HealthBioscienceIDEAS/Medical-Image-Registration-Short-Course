...
Therefore, You will first need to remove and pixels that contain NaN in either of the input images. You can use the *numpy* `isnan` function to find the indices of the NaN pixels in each image, and the `logical_or` function to combine these to get the indices of the pixels that are NaN in either image. You can then use boolean indexing to return 1D arrays containing all the pixel values from the images that were not NaN in either of the images. E.g.:
...
You can then calculate the mean and standard deviation of each image using the *numpy* `mean` and `std` functions. And then calculate the NCC using the equation from the lectures (the *numpy* `sum` function and the `size` attribute will be useful for this).
